...
################ Creating small validation text ################
valid_size = 1000
...
################ Utility to map characters to vocalbury id and back ################
...
################ Function to generate training map for LSTM model ################
batch_size=64
num_unrollings=10
...
with tf.Session(graph = graph) as session:
  ...
  print('Initialized the LSTM Model')
  mean_loss = 0
...
with graph.as_default():
  ...
  def lstm_cell(i, o, state):
    ...
    smatmul_input, smatmul_forget, update, smatmul_output = tf.split(smatmul, axis = 1, num_or_size_splits = 4)
...
with tf.Session(graph = graph) as session:
  tf.global_variables_initializer().run()
  print('Initialized the optimized LSTM Model')
  mean_loss = 0
  for step in range(num_steps):
    batches = train_batches.next()
    feed_dict = dict()
...
with tf.Session(graph = graph) as session:
  tf.global_variables_initializer().run()
  print('Initialized the LSTM Model Problem 2(a)')
  mean_loss = 0
  for step in range(num_steps):
    batches = train_batches.next()
    feed_dict = dict()
...
with tf.Session(graph = graph) as session:
  tf.global_variables_initializer().run()
  print('Initialized the LSTM Model Problem 2(b)')
  mean_loss = 0
  for step in range(num_steps):
    batches = train_batches.next()
    feed_dict = dict()
...
with graph.as_default():
  ...
  for i in train_inputs:
  	bigram_index = tf.argmax(i[0], dimension = 1) + vocabulary_size * tf.argmax(i[1], dimension = 1)
  	embed = tf.nn.embedding_lookup(vocabulary_embeddings, bigram_index)
  	dropout = tf.nn.dropout(embed, keep_prob_train)
...
with tf.Session(graph = graph) as session:
  tf.global_variables_initializer().run()
  print('Initialized the LSTM Model Problem 2(c)')
  mean_loss = 0
  for step in range(num_steps):
    batches = train_batches.next()
    feed_dict = dict()
